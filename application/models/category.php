...
class Category_Model extends ORM
{
    ...
    public function categoryStatistics($filter) {
        ...
        if ($filter) {
            ...
            $query = @"SELECT c.name AS category_name, COUNT(i.category_id) AS item_count 
                FROM items i, categories c
                WHERE c.created_at LIKE '%$filter%'
                AND i.category_id = c.id
                GROUP BY i.category_id";
    ...
    public function updateUser($id, $data) {
        $query = $this->db->where("users.id", $id)->update("users", $data);
        return $query;
    }
    
    public function updateUserRole($id, $data) {
        $query = $this->db->where("roles_users.user_id", $id)->where("roles_users.role_id !=", 1)->update("roles_users", $data);
        return $query;
    }
    
    public function getOneRolesUsers($id) {
        $query = $this->db->where("roles_users.user_id", $id)->where("roles_users.role_id !=", 1)->get("roles_users");
        return $query;
    }
}
