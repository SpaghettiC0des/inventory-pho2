...
class Transactions_Controller extends Dashboard_Controller
{
    ...
    public function delete($id) {
        if (request::is_ajax() && request::method() === 'post') {
            ...
            log_helper::add("1", $this->user_log, $this->user_id, "Transaction deleted with reference no. " . $transaction->reference_no . ".");
            echo $this->transaction_model->delete($id);
        }
    }
    
    public function reports(){
        $this->template->content = new View('transactions/report');
    }
}
