...
$(document).ready(function() {
     ...
     */
    window.notif = function notify(message, type) {
        $.growl({
            message: message
        }, {
            type: type,
    ...
    $("#site-favicon").fileinput({
        ...
        showUpload: false,
        initialPreview: [
            "<img src='" + image + "' class='file-preview-image' alt='Site Favicon' title='Site Favicon' width='213' height='160'>"
        ]
    });

    $("#additem-image").fileinput({
        browseClass: "btn btn-primary",
        showCaption: false,
        showRemove: false,
        showUpload: false,
    ...
    $("a#load_more").on("click", function() {
        var id = $(".postedlog:last").attr('id');
        $.post("logs/load_more_logs", {
            id: id
        }, function(data) {
            if (data) {
                $(".postedLogs").append(data);
            } else {
                $("a#load_more").html("No more logs!").attr('disabled', 'disabled');
            }
        });
    });
    ...
    $("form#system_setting").submit(function(event) {
        //event.preventDefault(); // Totally stop stuff happening
        var formData = new FormData($(this)[0]);
        $.ajax({
